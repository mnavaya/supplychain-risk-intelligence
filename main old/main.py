...
def load_and_clean_weekly_data(filename: str):
    ...
    print(f"✅ Loaded {len(df):,} rows with {df['sku'].nunique()} SKUs")
    
    df['week'] = pd.to_datetime(df['week'], errors='coerce')
    df = df.dropna(subset=['sku', 'inventory_level', 'consumption'])
    
    # Sort by SKU and week for proper time series
    ...
    print(f"✅ Data cleaned and ready in {time.time() - start:.2f} seconds")
    return df
...
def forecast_and_stockout(df, forecast_weeks=8):
    ...
    for sku in df['sku'].unique():
        ...
        # Use all available history for better forecasting
        if len(sku_data) < 3:
            forecast_demand = sku_data['consumption'].iloc[-1] if not sku_data.empty else 0
        else:
            model = ExponentialSmoothing(sku_data['consumption'], trend='add', seasonal=None, damped_trend=True)
            fit = model.fit()
        ...
        stockout_risk = "High" if days_to_stockout < (lead_time + 14) else "Medium" if days_to_stockout < 45 else "Low"
...
- Do not invent totals or previous week data.
- Explain anomalies using exact values (e.g., "fill rate 0.55 is below 0.80 threshold").
...
if __name__ == "__main__":
    ...
    sample_file = "week_2026-04-13_demo.csv"   # Your large file
